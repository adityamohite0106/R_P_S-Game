// Here i define possible choices for the game
...
// click on image to access event
let rock = document.getElementById("fist");
let paper = document.getElementById("hand");
let scissor = document.getElementById("scissor");
let pc;

//  localStorage (initial i kept 0)
let userScore = localStorage.getItem("userScore") ? parseInt(localStorage.getItem("userScore")) : 0;
let pcScore = localStorage.getItem("pcScore") ? parseInt(localStorage.getItem("pcScore")) : 0;

// Function to update user score and store it in localStorage
function changeUserScore(uscore) {
  localStorage.setItem("userScore", uscore);  // Save to localStorage
  document.getElementById("mscore").innerHTML = uscore;  // Update on the webpage
}

// Function to update computer score and store it in localStorage
function changePcScore(pscore) {
  localStorage.setItem("pcScore", pscore);  // Save to localStorage
  document.getElementById("cscore").innerHTML = pscore;  // Update on the webpage
}

// Initialize the displayed scores from localStorage when the page loads
window.onload = function () {
  changeUserScore(userScore);  // Display saved user score
  changePcScore(pcScore);  // Display saved PC score
}
...
rock.addEventListener("click", function () {
  ...
  if (pc === "rock") {
    ...
    lose();
    pcScore++;
    changePcScore(pcScore);  // Update and store the PC score
  } else {
    pcscissor.classList.remove("hidden");
    userrock.classList.add("rounded-gradient-borders");
    win();
    userScore++;
    changeUserScore(userScore);  // Update and store the user score
  }
});
